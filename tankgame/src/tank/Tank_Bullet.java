...
public class Tank_Bullet extends GameAnime {
    private final int speed = 20;
    private int index;
    private int[] angle;
    private int x, y;    
    private Picture_icon explosion, bullet_direction;
    ...
    public Tank_Bullet(Game_Panel gamePanel, Tank tank_a, Tank tank_b, Player player_a, Player player_b,
                       Wall wall, Picture_icon bullet_direction, int x, int y, int frame_delay, boolean loop) {
        ...
        try {            
           explosion = new Picture_icon("resource/Explosion_small_strip6.png", 24);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void explosion(Tank deadTank) {
        gamePanel.doAnimation(new GameAnime(this.explosion, deadTank.getImageX(), deadTank.getImageY(), 5, false));
    }
